...
def main():
    ...
    config_dir = "/u/mschwall/L7_SLC_off_files"
    config_file_name = config_dir + "/L7_SLC_off_config_file_NEX_master_v1.txt"
    expected_config_parms = 13 # total number of variable to be read from the config file 
    version_name = "L7_SLC_off_config_file_NEX_master_v1.txt"
    ##############################################################################
    ...
    my_exec_dir = config_dict["exec_dir"]  
    my_dir = config_dict["etm_multi_scene_dir"]
    ...
    job_name = PBS_job_name % str_time_date
    this_job_dir = my_exec_dir + job_name
    print " "
    print "***job directory and subdirectory names:"
    print ">>master directory name for this job"
    print this_job_dir

    # create a master directory for this job under the my_exec_dir
    ...
    input_files_dir = this_job_dir + "/" + PBS_job_name % "input_files"
    print ">>input_files_dir " 
    print input_files_dir
    if not os.path.exists(input_files_dir):
        os.makedirs(input_files_dir)
    # for log files
    log_files_dir = this_job_dir + "/" + PBS_job_name % "log_files"
    print ">>log_files_dir " 
    print log_files_dir
    ...
    # now read all of the Landsat scenes and store the scene directory names in Landsat_scenes
    print " "
    print "***reading Landsat scenes from:"
    print my_dir
    
    Landsat_scenes = []  # initialize the list
    for ix in os.listdir(my_dir):   # make a list of scene directories
        if (ix.find('LC8') >=0) or (ix.find('LCO') >= 0) or (ix.find('LE7') >=0):
            Landsat_scenes.append(ix)
        
    num_Landsat_scenes = len(Landsat_scenes)
    if num_Landsat_scenes <=0:  # bail out if there aren't any scenes found
        print "***did not find any Landsat scenes in this directory;"
        print my_dir
        return
    num_chunks = math.ceil(len(Landsat_scenes)/float(config_dict["scenes_per_node"]))
    print "***number of Landsat scenes: " + str(len(Landsat_scenes))
    print "***number of chunks: " + str(num_chunks)
    print " "
    print "***scene IDs:"
    for iz in Landsat_scenes: print iz
	...
	jobc_file.close()
        ...
        job_name_pbs = PBS_job_name % str(chunk_ctr) # PBS_job_name is specified in the config file
